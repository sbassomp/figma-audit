...
6. For **reach_paths** (IMPORTANT — this replaces the flat navigation_steps \
   ...
   \
   DETAIL-AFTER-LIST PATTERN (PREFERRED for entity-detail pages): when the \
   parametrised route is the detail of an entity that has a corresponding \
   LIST page in the same app (`/invoices/:id` <-> `/invoices`, `/orders/:id` \
   <-> `/orders`, `/messages/:id` <-> `/messages`), DO NOT seed the entity \
   via API and template the id. Use the listing instead: navigate to the \
   list, click the first tile, screenshot. This works without knowing any \
   backend DTO, works for entities the tool cannot create at all (invoices, \
   activity history, notifications, anything generated by a backend trigger), \
   and mirrors how a real user reaches the page. The shape: \
   ```json \
   ...
   ``` \
   The `min_y: 80` filter excludes the app bar back button so `index: 0` \
   lands on the first real list tile. ALWAYS prefer this pattern for \
   detail pages whose parent route is a listing. \
